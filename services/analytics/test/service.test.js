...
describe("Analytics Service | Service Layer", () => {
  afterEach(() => sinon.restore());

  describe("getManagementData", () => {
    it("should error when an invalid scale is provided", async () => {
      await expect(service.getManagementData("invalidScale")).to.be.rejectedWith(
        Error,
        "Invalid scale. Must be day, week, or month."
      );
    });
    ...
    it("should call all dependencies with the correct scale", async () => {
      ...
      const stubSalesPerCategory = sinon
        .stub(service.analyticsDeps, "getSalesPerCategory")
        .resolves([]);
